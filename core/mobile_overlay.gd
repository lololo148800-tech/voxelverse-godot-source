...
func _draw() -> void:
    ...
    _draw_survival_icons(viewport_size, scale_factor)
...
func _draw_survival_icons(viewport_size: Vector2, scale_factor: float) -> void:
    var icon_size := 18.0 * scale_factor
    var gap := 3.0 * scale_factor
    var left_origin := Vector2(18.0, 20.0) * scale_factor
    var right_origin := Vector2(viewport_size.x / scale_factor - (10.0 * icon_size + 9.0 * gap) - 18.0, 20.0) * scale_factor
    for index in range(10):
        var health_fill := health_ratio > float(index) / 10.0
        var hunger_fill := hunger_ratio > float(index) / 10.0
        _draw_heart(left_origin + Vector2((icon_size + gap) * index, 0.0), icon_size, health_fill)
        _draw_food_icon(right_origin + Vector2((icon_size + gap) * index, 0.0), icon_size, hunger_fill)

func _draw_heart(center: Vector2, size: float, filled: bool) -> void:
    var color := Color("e44755") if filled else Color(0.18, 0.16, 0.16, 0.72)
    var s := size * 0.5
    var points := PackedVector2Array([
        center + Vector2(-s * 0.95, -s * 0.18),
        center + Vector2(-s * 0.48, -s * 0.72),
        center + Vector2(0.0, -s * 0.30),
        center + Vector2(s * 0.48, -s * 0.72),
        center + Vector2(s * 0.95, -s * 0.18),
        center + Vector2(0.0, s * 0.92)
    ])
    draw_colored_polygon(points, color)
    draw_polyline(PackedVector2Array([points[0], points[1], points[2], points[3], points[4], points[5], points[0]]), Color(0.08, 0.06, 0.06, 0.86), maxf(1.0, size * 0.09))

func _draw_food_icon(center: Vector2, size: float, filled: bool) -> void:
    var color := Color("d59a3d") if filled else Color(0.20, 0.18, 0.14, 0.72)
    var s := size * 0.5
    draw_circle(center + Vector2(-s * 0.18, -s * 0.10), s * 0.30, color)
    draw_rect(Rect2(center + Vector2(-s * 0.04, -s * 0.30), Vector2(s * 0.24, s * 0.76)), color, true)
    draw_line(center + Vector2(s * 0.10, s * 0.34), center + Vector2(s * 0.46, s * 0.48), Color(0.12, 0.10, 0.08, 0.86), maxf(1.0, size * 0.08))

func _draw_top_strip(viewport_size: Vector2, scale_factor: float) -> void:
    var icon_size := 36.0 * scale_factor
...
func _draw_action_buttons(viewport_size: Vector2, scale_factor: float) -> void:
    var button_size := Vector2(56.0, 56.0) * scale_factor
    var right_x := viewport_size.x / scale_factor - 214.0
...
func _draw_first_person_hand(viewport_size: Vector2, scale_factor: float) -> void:
    var hand_scale := scale_factor * 0.62
    var base := Vector2(viewport_size.x / scale_factor - 142.0, viewport_size.y / scale_factor - 28.0) * scale_factor
    # Smaller blocky hand/sleeve silhouette, matching the compact reference framing.
    var sleeve := PackedVector2Array([base + Vector2(0.0, 50.0) * hand_scale, base + Vector2(18.0, 2.0) * hand_scale, base + Vector2(74.0, -22.0) * hand_scale, base + Vector2(122.0, 8.0) * hand_scale, base + Vector2(98.0, 66.0) * hand_scale, base + Vector2(20.0, 76.0) * hand_scale])
    draw_colored_polygon(sleeve, Color(0.20, 0.27, 0.48, 0.96))
    draw_polyline(sleeve, Color(0.08, 0.10, 0.16, 0.78), 2.0 * scale_factor)
    var cuff := PackedVector2Array([base + Vector2(34.0, 6.0) * hand_scale, base + Vector2(80.0, -12.0) * hand_scale, base + Vector2(105.0, 16.0) * hand_scale, base + Vector2(70.0, 35.0) * hand_scale])
